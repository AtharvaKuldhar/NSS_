...
const SignIn = () => {
    const router = useRouter();
    // Get the login function and the user state from our robust AuthContext
    ...
    // This effect will run when the `user` object in the context changes.
    // This is a more reliable way to handle redirection after login.
    useEffect(() => {
        if (user) {
            const redirectPath = user.userType === 0 ? "/admin/donations" : "/volunteer/donate";
            router.push(redirectPath);
        }
    }, [user, router]);


    // This is the corrected and simplified onSubmit function
    const onSubmit = async (data: z.infer<typeof signInSchema>) => {
        setIsSubmitting(true);
        try {
            // The login function from AuthContext now handles everything:
            // 1. Calls the login API.
            // 2. Fetches user data from /auth/me.
            // 3. Sets the user state globally.
            // 4. Shows a success toast.
            // The useEffect above will then handle the redirect.
            await login(data.email, data.password);
            
        } catch (error: any) {
            // Handle errors from the login process
            console.error("Error during sign in:", error);
            const errorMessage = error.response?.data?.message?.[0] || "Login failed. Please check your credentials.";
            toast.error("Sign in Failed", {
                description: errorMessage,
                richColors: true
            });
        ...
        <div className="flex justify-center items-center min-h-screen bg-gradient-to-t from-[#2C3E50]/90 to-white p-4">
            <div className="w-full max-w-4xl mx-auto flex rounded-2xl shadow-lg overflow-hidden bg-white">
                <div 
                    ...
                    style={{ backgroundImage: "url('/pict.jpg')" }}
                >
                    <div className="p-12 text-white bg-black bg-opacity-40 h-full flex flex-col justify-end">
                        <h2 className="text-4xl font-bold leading-tight">Share a Meal,</h2>
                        <p className="text-xl mt-2">Share a Hope.</p>
                    </div>
                </div>
                <div className="w-full md:w-1/2 p-8 md:p-12">
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                            ...
                            />
                            <Button type="submit" disabled={isSubmitting} className="w-full text-white font-bold bg-[#000000] hover:bg-[#024B05] py-6 text-md">
